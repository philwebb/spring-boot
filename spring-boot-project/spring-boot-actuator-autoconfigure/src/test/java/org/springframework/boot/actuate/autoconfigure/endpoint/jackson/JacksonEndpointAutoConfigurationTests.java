...
class JacksonEndpointAutoConfigurationTests {
	...
	void endpointObjectMapperWhenNoProperty() {
		this.runner.run((context) -> assertThat(context).doesNotHaveBean(EndpointObjectMapper.class));
	}
	...
	@Test
	void endpointObjectMapperWhenExistingEndpointObjectMapperBean() {
		this.runner.withUserConfiguration(TestEndpointMapperConfiguration.class)
				.run((context) -> assertThat(context.getBean(EndpointObjectMapper.class))
						.isInstanceOf(TestEndpointObjectMapper.class));
	}
