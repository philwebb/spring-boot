...
public class RequestMappingEndpointTests {
	...
	private WebMvcEndpointHandlerMapping createHandlerMapping() {
		WebOperationRequestPredicate requestPredicate = new WebOperationRequestPredicate(
				"test", WebEndpointHttpMethod.GET,
				Collections.singletonList("application/json"),
				Collections.singletonList("application/json"));
		WebOperation operation = mockOperation(requestPredicate);
		ExposableWebEndpoint endpoint = mock(ExposableWebEndpoint.class);
		given(endpoint.getId()).willReturn("test");
		given(endpoint.getOperations()).willReturn(Collections.singleton(operation));
		EndpointMapping endpointMapping = new EndpointMapping("actuator");
		EndpointMediaTypes endpointMediaTypes = new EndpointMediaTypes(
				Arrays.asList("application/vnd.test+json"),
				Arrays.asList("application/vnd.test+json"));
		WebMvcEndpointHandlerMapping mapping = new WebMvcEndpointHandlerMapping(
				endpointMapping, Collections.singleton(endpoint), endpointMediaTypes,
				null);
		mapping.setApplicationContext(new StaticApplicationContext());
	...
	@SuppressWarnings("unchecked")
	private WebOperation mockOperation(WebOperationRequestPredicate requestPredicate) {
		WebOperation operation = mock(WebOperation.class);
		given(operation.getType()).willReturn(OperationType.READ);
		given(operation.invoke(any(Map.class))).willReturn("Invoked");
		given(operation.getRequestPredicate()).willReturn(requestPredicate);
		return operation;
	}
