...
Application arguments can be specified using the `arguments` parameter, see xref:running.adoc#run.examples.using-application-arguments[using application arguments] for more details.
...
If you need to specify some JVM arguments (that is for debugging purposes), you can use the `jvmArguments` parameter, see xref:running.adoc#run.examples.debug[Debug the application] for more details.
There is also explicit support for xref:running.adoc#run.examples.system-properties[system properties] and xref:running.adoc#run.examples.environment-variables[environment variables].

As enabling a profile is quite common, there is dedicated `profiles` property that offers a shortcut for `-Dspring-boot.run.jvmArguments="-Dspring.profiles.active=dev"`, see xref:running.adoc#run.examples.specify-active-profiles[Specify active profiles].
