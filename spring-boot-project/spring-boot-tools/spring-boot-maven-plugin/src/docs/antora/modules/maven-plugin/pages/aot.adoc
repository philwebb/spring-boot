...
The `process-aot` goal shares a number of properties with the xref:running.adoc[run goal] for that reason.
...
* Suitable settings so that xref:packaging-oci-image.adoc[build-image] generates a native image.
