...
public abstract class AbstractPackagerMojo extends AbstractDependencyFilterMojo {
	 ...
	 * @since 3.1.0
	 */
	@Parameter(property = "spring-boot.repackage.excludeDevservices", defaultValue = "true")
	private boolean excludeDevServices = true;
	...
	private ArtifactsFilter[] getAdditionalFilters() {
		...
		if (this.excludeDevServices) {
			Exclude exclude = new Exclude();
			exclude.setGroupId("org.springframework.boot");
			exclude.setArtifactId("spring-boot-dev-services-docker-compose");
			ExcludeFilter filter = new ExcludeFilter(exclude);
			filters.add(filter);
		}
