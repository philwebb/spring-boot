...
class LifecycleVersion implements Comparable<LifecycleVersion> {
	...
	static LifecycleVersion parse(String value) {
		...
		Assert.isTrue(components.length <= 3, () -> "'value' must be a valid version number [" + value + "]");
		...
		for (int i = 0; i < components.length; i++) {
			try {
				...
				throw new IllegalArgumentException("'value' must be a valid version number [" + value + "]", ex);
			}
		}
