...
class HelpCommandTests {
	...
	void setup() {
		...
		this.command = new HelpCommand(context, ToolsJarMode.getCommands(context));
	...
	void runWhenHasNoCommandParameterPrintsUsage() {
		this.command.run(this.out, Collections.emptyMap(), Arrays.asList("dockerfile"));
		assertThat(this.out).hasSameContentAsResource("help-dockerfile-output.txt");
	}
