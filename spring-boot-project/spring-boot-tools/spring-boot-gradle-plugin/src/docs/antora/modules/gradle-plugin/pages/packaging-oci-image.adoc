...
| `imageName`
| `--imageName`
| xref:api:java/buildpack/platform/docker/type/ImageReference.html#of-java.lang.String-[Image name] for the generated image.
| `docker.io/library/${project.name}:${project.version}`

| `pullPolicy`
| `--pullPolicy`
| xref:api:java/buildpack/platform/build/PullPolicy.html[Policy] used to determine when to pull the builder and run images from the registry.
