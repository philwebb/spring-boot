...
[[spring-mvc.write-json-rest-service]]
...
[[spring-mvc.write-xml-rest-service]]
...
[[spring-mvc.customize-jackson-objectmapper]]
...
Note that, thanks to the use of xref:features/external-config/typesafe-configuration-properties/relaxed-binding.adoc[relaxed binding], the case of `indent_output` does not have to match the case of the corresponding enum constant, which is `INDENT_OUTPUT`.
...
See the "`xref:howto/spring-mvc.adoc#spring-mvc.customize-responsebody-rendering[Customize the @ResponseBody Rendering]`" section and the {spring-boot-autoconfigure-module-code}/web/servlet/WebMvcAutoConfiguration.java[`WebMvcAutoConfiguration`] source code for more details.



[[spring-mvc.customize-responsebody-rendering]]
...
[[spring-mvc.multipart-file-uploads]]
...
[[spring-mvc.switch-off-dispatcherservlet]]
...
[[spring-mvc.switch-off-default-configuration]]
...
[[spring-mvc.customize-view-resolvers]]
