...
. xref:io/caching/provider.adoc#io.caching.provider.generic[Generic]
. xref:io/caching/provider.adoc#io.caching.provider.jcache[JCache (JSR-107)] (EhCache 3, Hazelcast, Infinispan, and others)
. xref:io/caching/provider.adoc#io.caching.provider.hazelcast[Hazelcast]
. xref:io/caching/provider.adoc#io.caching.provider.infinispan[Infinispan]
. xref:io/caching/provider.adoc#io.caching.provider.couchbase[Couchbase]
. xref:io/caching/provider.adoc#io.caching.provider.redis[Redis]
. xref:io/caching/provider.adoc#io.caching.provider.caffeine[Caffeine]
. xref:io/caching/provider.adoc#io.caching.provider.cache2k[Cache2k]
. xref:io/caching/provider.adoc#io.caching.provider.simple[Simple]
...
Use this property if you need to xref:io/caching/provider.adoc#io.caching.provider.none[disable caching altogether] in certain environments (such as tests).
...
If you have multiple JCache compliant cache providers and want to force the use of Hazelcast, you have to xref:io/caching/provider.adoc#io.caching.provider.jcache[explicitly set the JCache provider].
...
== Couchbase
If Spring Data Couchbase is available and Couchbase is xref:data/nosql/couchbase.adoc[configured], a `CouchbaseCacheManager` is auto-configured.
