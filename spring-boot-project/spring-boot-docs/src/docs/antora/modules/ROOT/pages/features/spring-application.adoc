...
----
include::partial$application/spring-application.txt[]
----
...
Spring Boot can also be configured to expose a xref:api:rest/actuator/startup[`startup` endpoint] that provides this information as a JSON document.
