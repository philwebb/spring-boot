...
TIP: See xref:howto/data-access/configure-custom-datasource.adoc[the "`How-to`" section] for more advanced examples, typically to take full control over the configuration of the DataSource.
...
These are the standard options that work regardless of xref:data/sql/datasource.adoc#data.sql.datasource.connection-pool[the actual implementation].
