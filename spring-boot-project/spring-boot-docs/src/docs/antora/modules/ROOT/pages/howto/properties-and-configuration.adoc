...
TIP: If you want detailed information about the files that are being loaded you can xref:features/logging.adoc#features.logging.log-levels[set the logging level] of `org.springframework.boot.context.config` to `trace`.
...
See "`xref:features/external-config.adoc#features.external-config.yaml[Working With YAML]`" in the '`Spring Boot features`' section for more information about YAML.
...
Spring Boot supports multi-document YAML and Properties files (see xref:features/external-config.adoc#features.external-config.files.multi-document[Working With Multi-Document Files] for details) which can be activated conditionally based on the active profiles.
...
The appendix includes an xref:application-properties.adoc[`application.properties`] example with a list of the most common properties supported by Spring Boot.
