...
[[manual-hints.value-hint]]
== Value Hint
The `name` attribute of each hint refers to the `name` of a property.
In the xref:appendix/configuration-metadata/format.adoc[initial example shown earlier], we provide five values for the `spring.jpa.hibernate.ddl-auto` property: `none`, `validate`, `update`, `create`, and `create-drop`.
...
In order to offer additional content assistance for the keys, you could add the following JSON to xref:appendix/configuration-metadata/annotation-processor.adoc#annotation-processor.adding-additional-metadata[the manual metadata of the module]:
...
| `logger-name`
| Auto-completes valid logger names and xref:features/logging/log-groups.adoc[logger groups].
...
[[manual-hints.value-providers.any]]
=== Any
...
[[manual-hints.value-providers.class-reference]]
=== Class Reference
...
[[manual-hints.value-providers.handle-as]]
=== Handle As
...
[[manual-hints.value-providers.logger-name]]
=== Logger Name
The **logger-name** provider auto-completes valid logger names and xref:features/logging/log-groups.adoc[logger groups].
...
[[manual-hints.value-providers.spring-bean-reference]]
=== Spring Bean Reference
...
[[manual-hints.value-providers.spring-profile-name]]
=== Spring Profile Name
