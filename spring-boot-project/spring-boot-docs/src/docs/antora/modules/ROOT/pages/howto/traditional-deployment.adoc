...
If you use the xref:build-tool-plugins.adoc[Spring Boot build tools], marking the embedded servlet container dependency as provided produces an executable war file with the provided dependencies packaged in a `lib-provided` directory.
...
[[traditional-deployment.convert-existing-application]]
...
See the xref:howto/application.adoc#application.context-hierarchy[entry on building a hierarchy] for examples.
...
[[traditional-deployment.weblogic]]
