...
Spring Boot also provides an optional xref:build-tool-plugin:maven.adoc[Maven plugin] to create executable jars.
...
Spring Boot provides a useful xref:build-tool-plugin:gradle.adoc[Gradle plugin] that can be used to simplify dependency declarations and to create executable jars.
