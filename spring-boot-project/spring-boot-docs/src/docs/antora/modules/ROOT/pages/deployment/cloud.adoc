...
In this section, we look at what it takes to get the xref:getting-started/first-application.adoc[application that we developed] in the "`Getting Started`" section up and running in the Cloud.
...
[[cloud.cloud-foundry.binding-to-services]]
=== Binding to Services
...
Spring Boot helps you to xref:features/spring-application/application-availability.adoc[manage the state of your application] and export it with xref:actuator/endpoints/kubernetes-probes.adoc[HTTP Kubernetes Probes using Actuator].



[[cloud.kubernetes.container-lifecycle]]
...
[[cloud.aws.beanstalk.tomcat-platform]]
...
[[cloud.aws.beanstalk.java-se-platform]]
