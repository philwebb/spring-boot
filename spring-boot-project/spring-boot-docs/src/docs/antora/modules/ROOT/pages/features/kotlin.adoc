...
[[kotlin.configuration-properties]]
== @ConfigurationProperties
`@ConfigurationProperties` when used in combination with xref:features/external-config/typesafe-configuration-properties/constructor-binding.adoc[constructor binding] supports classes with immutable `val` properties as shown in the following example:
...
TIP: To generate xref:appendix/configuration-metadata/annotation-processor.adoc[your own metadata] using the annotation processor, {kotlin-docs}kapt.html[`kapt` should be configured] with the `spring-boot-configuration-processor` dependency.
...
If you need the `MockK` equivalent of the Mockito specific xref:features/testing/spring-boot-applications/mocking-beans.adoc[`@MockBean` and `@SpyBean` annotations], you can use https://github.com/Ninja-Squad/springmockk[SpringMockK] which provides similar `@MockkBean` and `@SpykBean` annotations.
...
[[kotlin.resources.further-reading]]
=== Further reading
