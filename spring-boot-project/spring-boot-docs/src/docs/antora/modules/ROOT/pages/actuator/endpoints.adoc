...
TIP: See {code-spring-boot-actuator-autoconfigure}/endpoint/web/CorsEndpointProperties.java[`CorsEndpointProperties`] for a complete list of options.
...
Health information is collected from the content of a {code-spring-boot-actuator}/health/HealthContributorRegistry.java[`HealthContributorRegistry`] (by default, all {code-spring-boot-actuator}/health/HealthContributor.java[`HealthContributor`] instances defined in your `ApplicationContext`).
...
| `cassandra`
| {code-spring-boot-actuator}/cassandra/CassandraDriverHealthIndicator.java[`CassandraDriverHealthIndicator`]
| Checks that a Cassandra database is up.

| `couchbase`
| {code-spring-boot-actuator}/couchbase/CouchbaseHealthIndicator.java[`CouchbaseHealthIndicator`]
| Checks that a Couchbase cluster is up.

| `db`
| {code-spring-boot-actuator}/jdbc/DataSourceHealthIndicator.java[`DataSourceHealthIndicator`]
| Checks that a connection to `DataSource` can be obtained.

| `diskspace`
| {code-spring-boot-actuator}/system/DiskSpaceHealthIndicator.java[`DiskSpaceHealthIndicator`]
| Checks for low disk space.

| `elasticsearch`
| {code-spring-boot-actuator}/elasticsearch/ElasticsearchRestClientHealthIndicator.java[`ElasticsearchRestClientHealthIndicator`]
| Checks that an Elasticsearch cluster is up.

| `hazelcast`
| {code-spring-boot-actuator}/hazelcast/HazelcastHealthIndicator.java[`HazelcastHealthIndicator`]
| Checks that a Hazelcast server is up.

| `influxdb`
| {code-spring-boot-actuator}/influx/InfluxDbHealthIndicator.java[`InfluxDbHealthIndicator`]
| Checks that an InfluxDB server is up.

| `jms`
| {code-spring-boot-actuator}/jms/JmsHealthIndicator.java[`JmsHealthIndicator`]
| Checks that a JMS broker is up.

| `ldap`
| {code-spring-boot-actuator}/ldap/LdapHealthIndicator.java[`LdapHealthIndicator`]
| Checks that an LDAP server is up.

| `mail`
| {code-spring-boot-actuator}/mail/MailHealthIndicator.java[`MailHealthIndicator`]
| Checks that a mail server is up.

| `mongo`
| {code-spring-boot-actuator}/data/mongo/MongoHealthIndicator.java[`MongoHealthIndicator`]
| Checks that a Mongo database is up.

| `neo4j`
| {code-spring-boot-actuator}/neo4j/Neo4jHealthIndicator.java[`Neo4jHealthIndicator`]
| Checks that a Neo4j database is up.

| `ping`
| {code-spring-boot-actuator}/health/PingHealthIndicator.java[`PingHealthIndicator`]
| Always responds with `UP`.

| `rabbit`
| {code-spring-boot-actuator}/amqp/RabbitHealthIndicator.java[`RabbitHealthIndicator`]
| Checks that a Rabbit server is up.

| `redis`
| {code-spring-boot-actuator}/data/redis/RedisHealthIndicator.java[`RedisHealthIndicator`]
...
| `livenessstate`
| {code-spring-boot-actuator}/availability/LivenessStateHealthIndicator.java[`LivenessStateHealthIndicator`]
| Exposes the "`Liveness`" application availability state.

| `readinessstate`
| {code-spring-boot-actuator}/availability/ReadinessStateHealthIndicator.java[`ReadinessStateHealthIndicator`]
...
To provide custom health information, you can register Spring beans that implement the {code-spring-boot-actuator}/health/HealthIndicator.java[`HealthIndicator`] interface.
...
In addition to Spring Boot's predefined {code-spring-boot-actuator}/health/Status.java[`Status`] types, `Health` can return a custom `Status` that represents a new system state.
In such cases, you also need to provide a custom implementation of the {code-spring-boot-actuator}/health/StatusAggregator.java[`StatusAggregator`] interface, or you must configure the default implementation by using the configprop:management.endpoint.health.status.order[] configuration property.
...
Similar to a traditional `HealthContributor`, health information is collected from the content of a {code-spring-boot-actuator}/health/ReactiveHealthContributorRegistry.java[`ReactiveHealthContributorRegistry`] (by default, all {code-spring-boot-actuator}/health/HealthContributor.java[`HealthContributor`] and {code-spring-boot-actuator}/health/ReactiveHealthContributor.java[`ReactiveHealthContributor`] instances defined in your `ApplicationContext`).
...
To provide custom health information from a reactive API, you can register Spring beans that implement the {code-spring-boot-actuator}/health/ReactiveHealthIndicator.java[`ReactiveHealthIndicator`] interface.
...
| `cassandra`
| {code-spring-boot-actuator}/cassandra/CassandraDriverReactiveHealthIndicator.java[`CassandraDriverReactiveHealthIndicator`]
| Checks that a Cassandra database is up.

| `couchbase`
| {code-spring-boot-actuator}/couchbase/CouchbaseReactiveHealthIndicator.java[`CouchbaseReactiveHealthIndicator`]
| Checks that a Couchbase cluster is up.

| `elasticsearch`
| {code-spring-boot-actuator}/data/elasticsearch/ElasticsearchReactiveHealthIndicator.java[`ElasticsearchReactiveHealthIndicator`]
| Checks that an Elasticsearch cluster is up.

| `mongo`
| {code-spring-boot-actuator}/data/mongo/MongoReactiveHealthIndicator.java[`MongoReactiveHealthIndicator`]
| Checks that a Mongo database is up.

| `neo4j`
| {code-spring-boot-actuator}/neo4j/Neo4jReactiveHealthIndicator.java[`Neo4jReactiveHealthIndicator`]
| Checks that a Neo4j database is up.

| `redis`
| {code-spring-boot-actuator}/data/redis/RedisReactiveHealthIndicator.java[`RedisReactiveHealthIndicator`]
| Checks that a Redis server is up.
|===
...
Application information exposes various information collected from all {code-spring-boot-actuator}/info/InfoContributor.java[`InfoContributor`] beans defined in your `ApplicationContext`.
...
| `build`
| {code-spring-boot-actuator}/info/BuildInfoContributor.java[`BuildInfoContributor`]
| Exposes build information.
| A `META-INF/build-info.properties` resource.

| `env`
| {code-spring-boot-actuator}/info/EnvironmentInfoContributor.java[`EnvironmentInfoContributor`]
| Exposes any property from the `Environment` whose name starts with `info.`.
| None.

| `git`
| {code-spring-boot-actuator}/info/GitInfoContributor.java[`GitInfoContributor`]
| Exposes git information.
| A `git.properties` resource.

| `java`
| {code-spring-boot-actuator}/info/JavaInfoContributor.java[`JavaInfoContributor`]
| Exposes Java runtime information.
| None.

| `os`
| {code-spring-boot-actuator}/info/OsInfoContributor.java[`OsInfoContributor`]
| Exposes Operating System information.
| None.

| `process`
| {code-spring-boot-actuator}/info/ProcessInfoContributor.java[`ProcessInfoContributor`]
...
To provide custom application information, you can register Spring beans that implement the {code-spring-boot-actuator}/info/InfoContributor.java[`InfoContributor`] interface.
