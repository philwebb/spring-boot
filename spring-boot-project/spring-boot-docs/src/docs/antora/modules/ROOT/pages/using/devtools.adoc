...
As described in the xref:using/devtools.adoc#using.devtools.restart.restart-vs-reload[null] section, restart functionality is implemented by using two classloaders.
...
For example, xref:web/servlet/spring-mvc.adoc#web.servlet.spring-mvc.template-engines[template engines] cache compiled templates to avoid repeatedly parsing template files.
...
As described earlier in the xref:using/devtools.adoc#using.devtools.restart.restart-vs-reload[null] section, restart functionality is implemented by using two classloaders.
