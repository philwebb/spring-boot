...
TIP: A list of the auto-configuration settings that are enabled by `@WebMvcTest` can be xref:appendix/test-auto-configuration.adoc[found in the appendix].
...
More details on how to use Spring Security's `MockMvc` support can be found in this _xref:howto/testing.adoc#testing.with-spring-security[Testing With Spring Security]_ how-to section.
