...
* Configuring codecs for `HttpMessageReader` and `HttpMessageWriter` instances (described xref:web/reactive/webflux.adoc#webflux.httpcodecs[later in this document]).
* Support for serving static resources, including support for WebJars (described xref:web/servlet/spring-mvc.adoc#spring-mvc.static-content[later in this document]).
...
You can also leverage xref:features/json.adoc#json.jackson.custom-serializers-and-deserializers[Boot's custom JSON serializers and deserializers].



[[webflux.static-content]]
== Static Content
...
You can also provide your own HTML templates to display errors (see the xref:web/reactive/webflux.adoc#webflux.error-handling.error-pages[next section]).
...
In some cases, errors handled at the controller or handler function level are not recorded by the xref:actuator/metrics/supported.adoc#supported.spring-webflux[metrics infrastructure].
...
[[webflux.error-handling.error-pages]]
=== Custom Error Pages
