...
| configprop:logging.structured.console[]
| `CONSOLE_LOG_STRUCTURED`
| The structured logging format to use for console logging.

| configprop:logging.structured.file[]
| `FILE_LOG_STRUCTURED`
...
To enable structured logging, set the property configprop:logging.structured.console[] (for console output) or configprop:logging.structured.file[] (for file output) to the id of the format you want to use.
...
To enable the Elastic Common Schema log format, set `logging.structured.console=ecs` or `logging.structured.file=ecs`.
...
To enable the Logstash JSON log format, set `logging.structured.console=logstash` or `logging.structured.file=logstash`.
...
To enable your custom format, set the property configprop:logging.structured.console[] or configprop:logging.structured.file[] to the fully qualified class name of your implementation.

Your implementation can use some constructor parameters, which are injected automatically.
Please see the JavaDoc of xref:api:java/org/springframework/boot/logging/structured/StructuredLoggingFormatter.html[`StructuredLoggingFormatter`] for more details.
