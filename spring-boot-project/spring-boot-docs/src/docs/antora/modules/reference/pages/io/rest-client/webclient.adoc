...
Spring Boot is configuring that builder to share HTTP resources, reflect codecs setup in the same fashion as the server ones (see xref:web/reactive/webflux.adoc#webflux.httpcodecs[WebFlux HTTP codecs auto-configuration]), and more.
