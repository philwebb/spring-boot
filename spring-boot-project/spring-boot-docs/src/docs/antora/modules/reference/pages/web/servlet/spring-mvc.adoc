...
* Support for serving static resources, including support for WebJars (covered xref:web/servlet/spring-mvc.adoc#spring-mvc.static-content[later in this document]).
* Automatic registration of `Converter`, `GenericConverter`, and `Formatter` beans.
* Support for `HttpMessageConverters` (covered xref:web/servlet/spring-mvc.adoc#spring-mvc.message-converters[later in this document]).
* Automatic registration of `MessageCodesResolver` (covered xref:web/servlet/spring-mvc.adoc#spring-mvc.message-codes[later in this document]).
* Static `index.html` support.
* Automatic use of a `ConfigurableWebBindingInitializer` bean (covered xref:web/servlet/spring-mvc.adoc#spring-mvc.binding-initializer[later in this document]).
...
[[spring-mvc.binding-initializer]]
...
There are several xref:web/servlet/embedded-container.adoc#embedded-container.jsp-limitations[known limitations] when using them with embedded servlet containers.
...
See the xref:appendix/application-properties#appendix.application-properties.server["`Server Properties`"] section of the Appendix.
...
In some cases, errors handled at the controller level are not recorded by the xref:actuator/metrics/supported.adoc#supported.spring-mvc[metrics infrastructure].
...
[[spring-mvc.error-handling.error-pages]]
=== Custom Error Pages
...
[[spring-mvc.error-handling.error-pages-without-spring-mvc]]
...
[[spring-mvc.error-handling.in-a-war-deployment]]
