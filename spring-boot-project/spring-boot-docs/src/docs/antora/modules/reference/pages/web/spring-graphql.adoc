...
[[graphql.transports.http-websocket]]
=== HTTP and WebSocket
...
[[graphql.transports.rsocket]]
=== RSocket
RSocket is also supported as a transport, on top of WebSocket or TCP.
Once the xref:messaging/rsocket.adoc#rsocket.server-auto-configuration[RSocket server is configured], we can configure our GraphQL handler on a particular route using configprop:spring.graphql.rsocket.mapping[].
