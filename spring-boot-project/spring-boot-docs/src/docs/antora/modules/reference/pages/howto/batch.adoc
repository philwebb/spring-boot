...
[[batch.running-from-the-command-line]]
== Running From the Command Line
Spring Boot converts any command line argument starting with `--` to a property to add to the `Environment`, see xref:features/external-config/command-line-args.adoc[accessing command line properties].
