...
xref:using/devtools.adoc#devtools.diagnosing-classloading-issues[Diagnosing Classloading Issues] explains how to diagnose and solve them.
...
If you want to use a xref:using/devtools.adoc#devtools.remote-applications[certain remote devtools feature], you need to include it.
...
[[devtools.diagnosing-classloading-issues]]
== Diagnosing Classloading Issues
As described in the xref:using/devtools.adoc#devtools.restart.restart-vs-reload[null] section, restart functionality is implemented by using two classloaders.
...
To diagnose whether the classloading issues are indeed caused by devtools and its two classloaders, xref:using/devtools.adoc#devtools.restart.disable[try disabling restart].
If this solves your problems, xref:using/devtools.adoc#devtools.restart.customizing-the-classload[customize the restart classloader] to include your entire project.
...
For example, xref:web/servlet/spring-mvc.adoc#spring-mvc.template-engines[template engines] cache compiled templates to avoid repeatedly parsing template files.
...
include::partial$using/devtools-property-defaults.adoc[]
...
Note that certain resources, such as static assets and view templates, xref:using/devtools.adoc#devtools.restart.excluding-resources[do not need to restart the application].
...
xref:using/devtools.adoc#devtools.livereload[See the LiveReload section] for details.
...
[[devtools.restart.restart-vs-reload]]
.Restart vs Reload
****
...
[[devtools.restart.logging-condition-delta]]
...
[[devtools.restart.excluding-resources]]
=== Excluding Resources
...
By default, changing resources in `/META-INF/maven`, `/META-INF/resources`, `/resources`, `/static`, `/public`, or `/templates` does not trigger a restart but does trigger a xref:using/devtools.adoc#devtools.livereload[live reload].
...
[[devtools.restart.watching-additional-paths]]
...
You can use the configprop:spring.devtools.restart.exclude[] property xref:using/devtools.adoc#devtools.restart.excluding-resources[described earlier] to control whether changes beneath the additional paths trigger a full restart or a xref:using/devtools.adoc#devtools.livereload[live reload].
...
TIP: You might want to set `spring.devtools.restart.trigger-file` as a xref:using/devtools.adoc#devtools.globalsettings[global setting], so that all your projects behave in the same way.
...
[[devtools.restart.customizing-the-classload]]
=== Customizing the Restart Classloader
As described earlier in the xref:using/devtools.adoc#devtools.restart.restart-vs-reload[null] section, restart functionality is implemented by using two classloaders.
...
WARNING: To trigger LiveReload when a file changes, xref:using/devtools.adoc#devtools.restart[Automatic Restart] must be enabled.



[[devtools.globalsettings]]
== Global Settings
...
For example, to configure restart to always use a xref:using/devtools.adoc#devtools.restart.triggerfile[trigger file], you would add the following property to your `spring-boot-devtools` file:
...
Any profiles activated in `.spring-boot-devtools.properties` will not affect the loading of xref:features/external-config/files/profile-specific.adoc[profile-specific configuration files].
...
[[devtools.globalsettings.configuring-file-system-watcher]]
...
[[devtools.remote-applications.client]]
...
[[devtools.remote-applications.update]]
=== Remote Update
The remote client monitors your application classpath for changes in the same way as the xref:using/devtools.adoc#devtools.restart[local restart].
...
If you observe such problems constantly, try increasing the `spring.devtools.restart.poll-interval` and `spring.devtools.restart.quiet-period` parameters to the values that fit your development environment.
See the xref:using/devtools.adoc#devtools.globalsettings.configuring-file-system-watcher[Configuring File System Watcher] section for configuring these properties.
