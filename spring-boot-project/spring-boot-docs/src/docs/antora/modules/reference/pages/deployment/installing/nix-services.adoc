...
If you configured Spring Boot's Maven or Gradle plugin to generate a xref:deployment/installing.adoc[fully executable jar], and you do not use a custom `embeddedLaunchScript`, your application can be used as an `init.d` service.
...
If root is used to control the application's service and you xref:deployment/installing/nix-services.adoc#nix-services.script-customization.when-running.conf-file[use a `.conf` file] to customize its startup, the `.conf` file is read and evaluated by the root user.
...
[[nix-services.script-customization]]
...
[[nix-services.script-customization.when-written]]
...
[[nix-services.script-customization.when-running]]
=== Customizing a Script When It Runs
For items of the script that need to be customized _after_ the jar has been written, you can use environment variables or a xref:deployment/installing/nix-services.adoc#nix-services.script-customization.when-running.conf-file[config file].
...
[[nix-services.script-customization.when-running.conf-file]]
...
To learn about securing this file appropriately, see xref:deployment/installing/nix-services.adoc#nix-services.init-d.securing[the guidelines for securing an init.d service].
