...
You can xref:actuator/endpoints/enabling.adoc[enable or disable] each individual endpoint and xref:actuator/endpoints/exposing.adoc[expose them (make them remotely accessible) over HTTP or JMX].
...
| `startup`
| Shows the xref:features/spring-application/startup-tracking.adoc[startup steps data] collected by the `ApplicationStartup`.
