...
For more detail, see the {code-spring-boot-actuator-autoconfigure}/web/server/ManagementServerProperties.java[`ManagementServerProperties`] source code and "`xref:ROOT:actuator/monitoring.adoc#actuator.monitoring.customizing-management-server-port[Customizing the Management Server Port]`" in the "`Production-ready features`" section.
...
See {code-spring-boot-autoconfigure}/web/servlet/error/ErrorMvcAutoConfiguration.java[`ErrorMvcAutoConfiguration`] for more options.

See also the section on "`xref:ROOT:web/servlet.adoc#web.servlet.spring-mvc.error-handling[Error Handling]`" for details of how to register handlers in the servlet container.
