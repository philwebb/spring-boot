...
TIP: See the xref:gradle-plugin:integrating-with-actuator[Spring Boot Gradle Plugin documentation] for more details.
...
To override dependency versions in Gradle, see xref:gradle-plugin:managing-dependencies.dependency-management-plugin.customizing[this section] of the Gradle plugin's documentation.
...
The executable archive cannot be used as a dependency as the xref:ROOT:executable-jar/nested-jars.adoc#appendix.executable-jar.nested-jars.jar-structure[executable jar format] packages application classes in `BOOT-INF/classes`.
