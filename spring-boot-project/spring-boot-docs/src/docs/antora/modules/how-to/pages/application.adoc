...
CAUTION: While using `@org.springframework.context.annotation.PropertySource` on your `@SpringBootApplication` may seem to be a convenient way to load a custom resource in the `Environment`, we do not recommend it.
