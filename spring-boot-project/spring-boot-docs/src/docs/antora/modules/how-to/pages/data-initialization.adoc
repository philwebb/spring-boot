...
This is done for you automatically if you enable the xref:ROOT:features/logging.adoc#features.logging.console-output[debug mode].
...
{code-spring-boot-autoconfigure}/flyway/FlywayProperties.java[`FlywayProperties`] provides most of Flyway's settings and a small set of additional properties that can be used to disable the migrations or switch off the location checking.
...
If you would like more control, provide a `@Bean` that implements {code-spring-boot-autoconfigure}/flyway/FlywayMigrationStrategy.java[`FlywayMigrationStrategy`].
...
See {code-spring-boot-autoconfigure}/liquibase/LiquibaseProperties.java[`LiquibaseProperties`] for details about available settings such as contexts, the default schema, and others.
