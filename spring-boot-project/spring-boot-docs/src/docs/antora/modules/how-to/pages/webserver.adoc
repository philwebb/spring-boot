...
For more details, see "`xref:ROOT:web/servlet.adoc#web.servlet.embedded-container.customizing[Customizing Embedded Servlet Containers]`" in the '`Spring Boot Features`' section, or the {spring-boot-autoconfigure-module-code}/web/ServerProperties.java[`ServerProperties`] source code.
...
Alternatively, the SSL trust material can be configured in an xref:ROOT:features/ssl.adoc[SSL bundle] and applied to the web server as shown in this example:
...
See the list of xref:ROOT:application-properties.adoc[Common Application Properties].
...
Like any other Spring bean, you can define the order of servlet filter beans; please make sure to check the "`xref:ROOT:web/servlet.adoc#web.servlet.embedded-container.servlets-filters-listeners.beans[Registering Servlets, Filters, and Listeners as Spring Beans]`" section.
...
See the xref:ROOT:application-properties.adoc#application-properties.server.server.tomcat.remoteip.internal-proxies[configprop:server.tomcat.remoteip.internal-proxies[] entry in the appendix] for its default value.
