...
If the class has a single parameterized constructor, one property is created per constructor parameter, unless the constructor is annotated with javadoc:{url-spring-framework-javadoc}/org.springframework.beans.factory.annotation.Autowired[format=annotation].
...
The annotation processor also supports the use of the `@Data`, javadoc:{url-spring-framework-javadoc}/org.springframework.beans.factory.annotation.Value[format=annotation], javadoc:{url-hibernate-javadoc}/org.hibernate.property.access.spi.Getter[format=annotation], and javadoc:{url-hibernate-javadoc}/org.hibernate.property.access.spi.Setter[format=annotation] lombok annotations.
