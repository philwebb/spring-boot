...
| The class name of the data type of the group.
  ...
  If it were based on a javadoc:{url-spring-framework-javadoc}/org.springframework.context.annotation.Bean[format=annotation] method, it would be the return type of that method.
...
| `providers`
| ValueProvider[]
| A list of providers as defined by the javadoc:{url-spring-data-mongodb-javadoc}/org.springframework.data.mongodb.util.json.ValueProvider[] object (described later in this document).
