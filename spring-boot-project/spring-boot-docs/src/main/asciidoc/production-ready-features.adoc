...
A `*` can be used to indicate all endpoints. For example, to expose everything over HTTP
...
NOTE: If your application is exposed public we strongly recommend that you also
...
also register an `EndpointFilter` bean.
...
also available on `EndpointRequest` (see the Javadoc for details).
...
[source,properties,indent=0]
----
	management.endpoint.beans.time-to-live=10
...
disables _all_ endpoints except for `info`:
...
[[production-ready-customizing-web-endpoints]]
==== Customizing Web Endpoints


[[production-ready-customizing-jmx-endpoints]]
==== Customizing JMX Endpoints
