...
----
	...
	spring.redis.lettuce.pool.max-wait=-1 # Maximum amount of time a connection allocation should block before throwing an exception when the pool is exhausted. Use a negative value to block indefinitely.
	...
	management.cloudfoundry.skip-ssl-validation=false # Whrther to skip SSL verification for Cloud Foundry actuator endpoint security calls.
