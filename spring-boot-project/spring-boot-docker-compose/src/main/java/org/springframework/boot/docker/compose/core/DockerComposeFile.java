...
public final class DockerComposeFile {
	...
	public static DockerComposeFile of(File file) {
		...
		Assert.isTrue(file.exists(), () -> "'file' must exist [%s]".formatted(file));
		Assert.isTrue(file.isFile(), () -> "'file' must be a regular file [%s]".formatted(file));
	...
	public static DockerComposeFile of(Collection<? extends File> files) {
		...
		for (File file : files) {
			...
			Assert.isTrue(file.exists(), () -> "'files' content must exist [%s]".formatted(file));
			Assert.isTrue(file.isFile(), () -> "'files' content must be regular files [%s]".formatted(file));
		}
