...
class StructuredLoggingJsonPropertiesTests {
	...
	void getWhenHasStackTracePropertiesBindsFromEnvironment() {
		...
		environment.setProperty("logging.structured.json.stacktrace.show-common-frames", "true");
		environment.setProperty("logging.structured.json.stacktrace.single-line", "true");
		StructuredLoggingJsonProperties properties = StructuredLoggingJsonProperties.get(environment);
	...
	class StackTraceTests {
		...
		void createPrinterWhenStandardAppliesCustomizations() {
			...
			StackTrace properties = new StackTrace(null, Root.FIRST, 300, 2, true, true);
			...
			assertThat(actual).isEqualTo("java.lang.RuntimeException: exception\\n"
		...
		void createPrinterWhenClassNameCreatesPrinter() {
			Exception exception = TestException.create();
			StackTrace properties = new StackTrace(TestStackTracePrinter.class.getName(), null, null, null, true, true);
			StackTracePrinter printer = properties.createPrinter();
