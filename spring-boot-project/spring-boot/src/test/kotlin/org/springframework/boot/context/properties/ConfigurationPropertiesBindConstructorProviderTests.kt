...
class ConfigurationPropertiesBindConstructorProviderTests {
	...
	@Test
	fun `type with primary constructor and no autowired should register value object`() {
		val bindConstructor = this.constructorProvider.getBindConstructor(BarProperties::class.java, false)
		assertThat(bindConstructor).isNotNull()
	}

	@Test
	fun `type with no primary constructor should register java bean`() {
		val bindConstructor = this.constructorProvider.getBindConstructor(BingProperties::class.java, false)
		assertThat(bindConstructor).isNull()
	}
	...
	@Test
	fun `type with constructor binding on type and autowired primary constructor should throw exception`() {
		assertThatIllegalStateException().isThrownBy { this.constructorProvider.getBindConstructor(ConstructorBindingOnTypeAndAutowiredPrimaryProperties::class.java, false) }
	}

	@Test
	fun `type with constructor binding on type and autowired secondary constructor should throw exception`() {
		assertThatIllegalStateException().isThrownBy {
			this.constructorProvider.getBindConstructor(ConstructorBindingOnTypeAndAutowiredSecondaryProperties::class.java, false
			)
		}
	}
	...
	@ConstructorBinding
	@ConfigurationProperties(prefix = "bar")
	class ConstructorBindingOnTypeAndAutowiredPrimaryProperties @Autowired constructor(val name: String?, val counter: Int = 42)

	@ConstructorBinding
	@ConfigurationProperties(prefix = "bar")
	class ConstructorBindingOnTypeAndAutowiredSecondaryProperties {
