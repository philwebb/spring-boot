...
public class StructuredLogFormatterFactory<E> {
	...
	private List<StructureLoggingJsonMembersCustomizer<?>> loadStructureLoggingJsonMembersCustomizers() {
		return (List) SpringFactoriesLoader.forDefaultResourceLocation()
			.load(StructureLoggingJsonMembersCustomizer.class, ArgumentResolver.from(this.instantiator::getArg));
	}
	...
	private StructuredLogFormatter<E> getUsingClassName(String className) {
		StructuredLogFormatter<E> formatter = (StructuredLogFormatter<E>) this.instantiator.instantiate(className);
		if (formatter != null) {
