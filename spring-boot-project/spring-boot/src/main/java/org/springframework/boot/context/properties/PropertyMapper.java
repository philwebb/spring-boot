...
public final class PropertyMapper {
	...
	public static class Source<T> {
		 ...
		 */
		public <D extends Number> Source<Integer> asInt(Function<T, D> adapter) {
		 ...
		 * @return a new adapted source instance
		 */
		public <D> Source<D> as(Function<T, D> adapter) {
			Assert.notNull(adapter, "Adapter must not be null");
			Supplier<D> supplier = () -> adapter.apply(this.supplier.get());
			Predicate<D> predicate = (t) -> this.predicate.test(this.supplier.get());
			return new Source<D>(supplier, predicate);
		}
		 ...
		 */
		public <D> D toInstance(Function<T, D> factory) {
