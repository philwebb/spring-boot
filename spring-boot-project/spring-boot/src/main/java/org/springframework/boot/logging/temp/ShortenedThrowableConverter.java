 ...
 */
package org.springframework.boot.logging.temp;
...
/**
 * A {@link ThrowableHandlingConverter} (similar to logback's
 * {@link ThrowableProxyConverter}) that formats stacktraces by doing the following:
 *
 * <ul>
 * <li>Limits the number of stackTraceElements per throwable (applies to each individual
 * throwable. e.g. caused-bys and suppressed). See {@link #maxDepthPerThrowable}.</li>
 * <li>Limits the total length in characters of the trace. See {@link #maxLength}.</li>
 * <li>Abbreviates class names based. See {@link #setShortenedClassNameLength(int)}.</li>
 ...
 * regular expression. See {@link #truncateAfterPatterns}.
 * <li>Uses evaluators to determine if the stacktrace should be logged. See
 * {@link #evaluators}.</li>
 * <li>Outputs in either 'normal' order (root-cause-last), or root-cause-first. See
 * {@link #rootCauseFirst}.</li>
 * </ul>
 *
 * To use this with a {@link PatternLayout}, you must configure {@code conversionRule} as
 * described
 * <a href="http://logback.qos.ch/manual/layouts.html#customConversionSpecifier">here</a>.
 ...
 * <li>"inlineHash" - indicating that hexadecimal error hashes should be computed and
 * inlined
 ...
 * <li>evaluator name - name of evaluators that will determine if the stacktrace is
 * ignored
...
public class ShortenedThrowableConverter extends ThrowableHandlingConverter {
	 ...
	 * {@link CoreConstants#LINE_SEPARATOR} when inline is active
	 */
	 ...
	 *
	 * The strings being matched against are in the form
	 * "fullyQualifiedClassName.methodName" (e.g. "java.lang.Object.toString").
	 *
	 * Note that these elements will only be excluded if and only if more than one
	 * consecutive line matches an exclusion pattern.
	 */
	...
	/**
	 * True to print the root cause first. False to print exceptions normally (root cause
	 * last).
	 */
	...
	public void start() {
		...
		// FIXME LogbackUtils.start(getContext(), this.abbreviator);
		super.start();
	}
	...
	public void stop() {
		super.stop();
		// FIXME LogbackUtils.stop(this.abbreviator);
	}
		...
		for (int i = 0; i < optionListSize; i++) {
			...
			switch (i) {
				case OPTION_INDEX_MAX_DEPTH:
					setMaxDepthPerThrowable(parseIntegerOptionValue(option, FULL_MAX_DEPTH_PER_THROWABLE,
							SHORT_MAX_DEPTH_PER_THROWABLE, DEFAULT_MAX_DEPTH_PER_THROWABLE));
					break;
				case OPTION_INDEX_SHORTENED_CLASS_NAME:
					setShortenedClassNameLength(parseIntegerOptionValue(option, FULL_CLASS_NAME_LENGTH,
							SHORT_CLASS_NAME_LENGTH, DEFAULT_CLASS_NAME_LENGTH));
					break;
				case OPTION_INDEX_MAX_LENGTH:
					setMaxLength(
							parseIntegerOptionValue(option, FULL_MAX_LENGTH, SHORT_MAX_LENGTH, DEFAULT_MAX_LENGTH));
					break;
				default:
					/*
					 * Remaining options are either - "rootFirst" - indicating that stacks
					 * should be printed root-cause first - "inlineHash" - indicating that
					 * hexadecimal error hashes should be computed and inlined - "inline"
					 * - indicating that the whole stack trace should be inlined, using
					 * "\\n" as separator - "omitCommonFrames" - omit common frames -
					 * "keepCommonFrames" - keep common frames - evaluator name - name of
					 * evaluators that will determine if the stacktrace is ignored -
					...
					switch (option) {
						...
						default:
							...
							EventEvaluator<ILoggingEvent> evaluator = (evaluatorMap != null)
									? (EventEvaluator<ILoggingEvent>) evaluatorMap.get(option) : null;
	...
	public String convert(ILoggingEvent event) {
		...
		if (this.inlineHash && (throwableProxy instanceof ThrowableProxy)) {
			// PW Calculate hash on the original throwable
			stackHashes = this.stackHasher.hexHashes(((ThrowableProxy) throwableProxy).getThrowable());
		}

		/*
		 * The extra 100 gives a little more buffer room since we actually go over the
		 * maxLength before detecting it and truncating.
		 */
		StringBuilder builder = new StringBuilder(
				Math.min(BUFFER_INITIAL_CAPACITY, this.maxLength + 100 > 0 ? this.maxLength + 100 : this.maxLength));

		// PW switch on root cause first/last
		...
		// PW Truncate if too long (with ...). The append method stops as well but might
		// be slightly over
	...
	/**
	 * Return true if any evaluator returns true, indicating that the stack trace should
	 * not be logged.
	 */
	...
	/**
	 * Appends a throwable and recursively appends its causedby/suppressed throwables in
	 * "normal" order (Root cause last).
	 */
	private void appendRootCauseLast(StringBuilder builder, String prefix, int indent, IThrowableProxy throwableProxy,
		...
		// PW this is the usual format

		if (throwableProxy == null || builder.length() > this.maxLength) {
			return; // PW bail at end or if too long
		}
		...
		if (suppressedThrowableProxies != null) {
			...
			for (IThrowableProxy suppressedThrowableProxy : suppressedThrowableProxies) {
				...
				// Think hashing is calculated because it uses parent
				appendRootCauseLast(builder, CoreConstants.SUPPRESSED,
						indent + ThrowableProxyUtil.SUPPRESSED_EXCEPTION_INDENT, suppressedThrowableProxy, null);
			}
		}

		// PW recurse with cause
		appendRootCauseLast(builder, CoreConstants.CAUSED_BY, indent, throwableProxy.getCause(), stackHashes);
	}

	/**
	 * Appends a throwable and recursively appends its causedby/suppressed throwables in
	 * "reverse" order (Root cause first).
	 */
	private void appendRootCauseFirst(StringBuilder builder, String prefix, int indent, IThrowableProxy throwableProxy,
		...
		if (suppressedThrowableProxies != null) {
			for (IThrowableProxy suppressedThrowableProxy : suppressedThrowableProxies) {
				// stack hashes are not computed/inlined on suppressed errors
				appendRootCauseFirst(builder, CoreConstants.SUPPRESSED,
						indent + ThrowableProxyUtil.SUPPRESSED_EXCEPTION_INDENT, suppressedThrowableProxy, null);
			}
		}
	}
	...
	private void appendStackTraceElements(StringBuilder builder, int indent, IThrowableProxy throwableProxy) {
		...
		for (; i < stackTraceElements.length - commonFrames; i++) {
			if (this.maxDepthPerThrowable > 0 && appended >= this.maxDepthPerThrowable) {
				 ...
				 */
				break;
				// PW max depth per target is number to bail at
			}
			StackTraceElementProxy stackTraceElement = stackTraceElements[i];
			if (i < 1 || isIncluded(stackTraceElement)) { // First 2 frames are always
															// included
				/*
				 * We should append this line.
				 *
				 * consecutiveExcluded will be > 0 if we were previously skipping lines
				 * based on excludes
				 */
				if (consecutiveExcluded >= 2) {
					...
					/*
					 * We only excluded one line, so just go back and include it instead
					 * of printing the excluding message for it.
					 */
		...
		if (i + commonFrames < stackTraceElements.length) {
			/*
			 * We were excluding elements but we want the truncateAfter element to be
			 * printed
			 */
			...
			if (commonFrames > 0) {
				appendPlaceHolder(builder, indent, stackTraceElements.length - appended - consecutiveExcluded,
	...
	/**
	 * Return {@code true} if the stack trace element is included (i.e. doesn't match any
	 * exclude patterns).
	...
	/**
	 * Return {@code true} if the stacktrace should be truncated after the element passed
	 * as argument
	...
	private void appendStackTraceElement(StringBuilder builder, int indent, StackTraceElementProxy step,
		...
		builder.append("at ")
			.append(this.abbreviator.abbreviate(stackTraceElement.getClassName()))
			.append(".")
	 ...
	 *
	 * Packaging data for the current step is only appended if it differs from the
	 * packaging data from the previous step.
	 */
	...
	private void appendFirstLine(StringBuilder builder, String prefix, int indent, IThrowableProxy throwableProxy,
		...
		builder.append(this.abbreviator.abbreviate(throwableProxy.getClassName()))
			.append(": ")
	...
	/**
	 * Set the length to which class names should be abbreviated. Cannot be used if a
	 * custom {@link Abbreviator} has been set through
	 * {@link #setClassNameAbbreviator(Abbreviator)}.
	 * @param length the desired maximum length or {@code -1} to disable the feature and
	 * allow for any arbitrary length.
	 */
	...
	/**
	 * Get the class name abbreviation target length. Cannot be used if a custom
	 * {@link Abbreviator} has been set through
	 * {@link #setClassNameAbbreviator(Abbreviator)}.
	...
	/**
	 * Set a limit on the number of stackTraceElements per throwable. Use {@code -1} to
	 * disable the feature and allow for an unlimited depth.
	 * @param maxDepthPerThrowable the maximum number of stacktrace elements per throwable
	 * or {@code -1} to disable the feature and allows for an unlimited amount.
	 */
	 ...
	 * @param maxLength the maximum size of the rendered stacktrace or {@code -1} for no
	 * limit.
	 */
