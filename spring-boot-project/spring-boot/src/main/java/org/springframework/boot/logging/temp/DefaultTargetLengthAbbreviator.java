 ...
 *
 * <p>
 * The algorithm will shorten the full class name and attempt to reduce its size to a
 * maximum of {@code length} characters. It does so by reducing the package elements to
 * their first letter and gradually expand them starting from the right until the maximum
 * size is reached. Setting the length to zero constitutes an exception and returns the
