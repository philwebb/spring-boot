...
public class InvalidConfigDataPropertyException extends ConfigDataException {
	...
	private static final Map<ConfigurationPropertyName, ConfigurationPropertyName> INVALID;
	static {
		Map<ConfigurationPropertyName, ConfigurationPropertyName> invalid = new LinkedHashMap<>();
		invalid.put(ConfigurationPropertyName.of("spring.profile"),
				ConfigurationPropertyName.of("spring.config.activate.on-profile"));
		INVALID = Collections.unmodifiableMap(invalid);
	}
	 ...
	 */
	static void throwIfInvalidPropertyFound(ConfigDataEnvironmentContributor contributor) {
		ConfigurationPropertySource propertySource = contributor.getConfigurationPropertySource();
		if (propertySource != null) {
			INVALID.forEach((invalid, replacement) -> {
