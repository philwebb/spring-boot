...
public class DefaultBootstrapRegistry implements BootstrapRegistry {
	...
	private <T> Supplier<T> register(Class<T> type, Registration<T> registration, boolean replaceExisting) {
		...
		synchronized (this.registrations) {
			Assert.state(replaceExisting || !this.instances.containsKey(type),
					() -> type.getName() + " has already been created");
	...
	void applicationContextPrepared(ConfigurableApplicationContext applicationContext) {
		for (ApplicationContextPreparedListener listener : this.applicationContextPreparedListeners) {
			listener.run(this, applicationContext);
		}
	}
	...
	void clear() {
		synchronized (this.registrations) {
			...
			this.applicationContextPreparedListeners.clear();
		}
	}
