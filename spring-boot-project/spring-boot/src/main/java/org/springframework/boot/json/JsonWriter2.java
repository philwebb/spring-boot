 ...
 */
@FunctionalInterface
public interface JsonWriter2<T> {
	...
	default JsonWriter2<T> endingWithNewLine() {
		return endingWith("\n");
	}

	default JsonWriter2<T> endingWith(String suffix) {
		return (instance, out) -> {
			write(instance, out);
			append(out, suffix);
		};
	...
	static <T> JsonWriter2<T> of(Consumer<Members<T>> members) {
		return accept(members, new Members<>(), Members::writer);
	}

	// FIXME listOf() arrayOf()

	static <T> JsonWriter2<T> using(BiConsumer<T, ValueWriter> writer) {
	...
	public static class ValueWriter {
		...
		private void append(char ch) {
			JsonWriter2.append(this.out, ch);
		}

		private void append(CharSequence value) {
			JsonWriter2.append(this.out, value);
		}
	...
	public static final class Members<T> {
		...
		JsonWriter2<T> writer() {
			List<Member.Writer> memberWriters = this.members.stream().map(Member::writer).toList();
			return JsonWriter2.using((instance, valueWriter) -> valueWriter.writePairs((pairs) -> {
	...
	public static final class Member<T> {
		...
		public Member<T> usingMembers(Consumer<Members<T>> members) {
			JsonWriter2<T> membersWriter = JsonWriter2.of(members);
