	...
	static class EmbeddedDataSourceBeanFactoryPostProcessor implements BeanDefinitionRegistryPostProcessor {
		...
		private boolean isUsingTestServiceConnection(ConfigurableListableBeanFactory beanFactory) {
			for (String beanName : beanFactory.getBeanNamesForType(JdbcConnectionDetails.class)) {
				try {
					if (isTestServiceConnection(beanFactory.getBeanDefinition(beanName))) {
						return true;
					}
				}
		...
		private boolean isTestServiceConnection(BeanDefinition beanDefinition) {
			String className = beanDefinition.getBeanClassName();
			// FIXME Would it be better to have an attibute set on the bean definition to
			// save us knowing about classes from other jars
			return (className != null) && (className.startsWith("org.springframework.boot.testcontainers.")
					|| className.startsWith("org.springframework.boot.docker.compose.service.connection."));
		}
