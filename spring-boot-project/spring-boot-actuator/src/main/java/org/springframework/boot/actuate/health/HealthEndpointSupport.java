...
abstract class HealthEndpointSupport<C, T> {
	...
	HealthResult<T> getHealthResult(SecurityContext securityContext, boolean alwaysIncludeDetails, String... path) {
	...
	private HealthResult<T> getHealth(HealthEndpointGroup group, SecurityContext securityContext,
		...
		T health = getHealth(group, contributor, includeDetails, isSystemHealth ? this.groups.getNames() : null);
	...
	@SuppressWarnings("unchecked")
	private T getHealth(HealthEndpointGroup group, Object contributor, boolean includeDetails, Set<String> groupNames) {
		if (contributor instanceof NamedContributors) {
	...
	private T getAggregateHealth(HealthEndpointGroup group, NamedContributors<C> namedContributors,
		...
		for (NamedContributor<C> namedContributor : namedContributors) {
			...
			if (group.isMember(name)) {
				T contribution = getHealth(group, namedContributor.getContributor(), includeDetails, null);
		...
		return aggregate(contributions, group.getStatusAggregator(), includeDetails, groupNames);
	}

	protected abstract T getHealth(C contributor, boolean includeDetails);

	protected abstract T aggregate(Map<String, T> contributions, StatusAggregator statusAggregator,
			boolean includeDetails, Set<String> groupNames);
