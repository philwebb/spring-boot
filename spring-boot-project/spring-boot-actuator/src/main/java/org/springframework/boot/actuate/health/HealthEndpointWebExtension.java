...
public class HealthEndpointWebExtension extends HealthEndpointSupport<HealthContributor, HealthComponent> {
	...
	public WebEndpointResponse<HealthComponent> health(SecurityContext securityContext, boolean alwaysIncludeDetails,
			String... path) {
		HealthResult<HealthComponent> result = getHealthResult(securityContext, alwaysIncludeDetails, path);
		if (result == null) {
	...
	@Override
	protected HealthComponent aggregate(Map<String, HealthComponent> contributions, StatusAggregator statusAggregator,
