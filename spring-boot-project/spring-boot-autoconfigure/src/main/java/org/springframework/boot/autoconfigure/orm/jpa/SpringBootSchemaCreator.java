...
class SpringBootSchemaCreator implements SchemaCreator {
	...
	public void doCreation(Metadata metadata, ExecutionOptions options, SourceDescriptor sourceDescriptor,
		...
		try {
			doBeforeCreation(databaseTarget);
			this.creator.doCreation(metadata, options, sourceDescriptor, targetDescriptor);
			doAfterCreation(databaseTarget);
		}
		finally {
			try {
	...
	private void doBeforeCreation(GenerationTarget databaseTarget) {
		System.err.println("Run schema.sql");
	}

	private void doAfterCreation(GenerationTarget databaseTarget) {
		System.err.println("Run data.sql");
		databaseTarget.accept(
				"insert into hotel(id, city_id, name, address, zip) values (4000, 3, 'TestTest', 'TEST', '2000')");
		System.err.println("Done running data.sql");
	}
