...
class AutoConfiguredHttpServiceProxyFactoryProvider implements HttpServiceProxyFactoryProvider {
	...
	private List<HttpServiceProxyFactoryProvider> providers;
	...
	@Override
	public @Nullable HttpServiceProxyFactory getHttpServiceProxyFactory(String groupId) {
		HttpServiceProxyFactory result = null;
		for (HttpServiceProxyFactoryProvider provider : this.providers) {
			HttpServiceProxyFactory candidate = provider.getHttpServiceProxyFactory(groupId);
			Assert.state(result == null,
					() -> "Multiple HttpServiceProxyFactoryProviders found supporting group ID '%s'"
						.formatted(groupId));
			result = candidate;
		}
		return result;
