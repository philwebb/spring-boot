...
public class LiquibaseAutoConfiguration {
	...
	public static class LiquibaseConfiguration {
		...
		private SpringLiquibase createSpringLiquibase(DataSource liquibaseDataSource, DataSource dataSource) {
			DataSource migrationDataSource = (liquibaseDataSource != null) ? liquibaseDataSource : dataSource;
			boolean existingDataSource = true;
			...
			if (properties.getUrl() != null) {
				...
				migrationDataSource = builder.build();
				existingDataSource = false;
			}
			else if (properties.getUser() != null && migrationDataSource != null) {
				DataSourceBuilder<?> builder = DataSourceBuilder.derivedFrom(migrationDataSource);
				applyCommonBuilderProperties(properties, builder);
				migrationDataSource = builder.build();
				existingDataSource = false;
			}
			Assert.state(migrationDataSource != null, "Liquibase migration DataSource missing");
			SpringLiquibase liquibase = (!existingDataSource) ? new DataSourceClosingSpringLiquibase()
					: new SpringLiquibase();
