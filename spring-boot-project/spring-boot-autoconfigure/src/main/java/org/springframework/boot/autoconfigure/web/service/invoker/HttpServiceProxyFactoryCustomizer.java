...
public interface HttpServiceProxyFactoryCustomizer extends Selector<HttpServiceProxyFactoryCustomizer> {
	 ...
	 * Callback to customize a
	 * {@link org.springframework.web.service.invoker.HttpServiceProxyFactory.Builder
	 * HttpServiceProxyFactory.Builder} instance for a selected {@link Selectable}.
	 * @param httpServiceProxyFactoryBuilder the HTTP service proxy factory builder to
	 * customize
	 * @param selected the {@link #selects(Selectable) selected} selectable
	 */
	default void customize(HttpServiceProxyFactory.Builder httpServiceProxyFactoryBuilder, Selectable selected) {
		customize(httpServiceProxyFactoryBuilder);
	}
	...
	default HttpServiceProxyFactoryCustomizer onlyWhen(Predicate<Selectable> predicate) {
		return new ForSelected() {
			...
			@Override
			public void customize(HttpServiceProxyFactory.Builder httpServiceProxyFactoryBuilder, Selectable selected) {
				HttpServiceProxyFactoryCustomizer.this.customize(httpServiceProxyFactoryBuilder, selected);
			}
	 ...
	 * @return the customizer
	 */
	static HttpServiceProxyFactoryCustomizer of(ForSelected customizer) {
		return customizer;
	}

	/**
	 * {@link HttpServiceProxyFactoryCustomizer} that can be used when the selected
	 * {@link Selectable} is needed during customization`.
	 */
	interface ForSelected extends HttpServiceProxyFactoryCustomizer {

		@Override
		default void customize(Builder httpServiceProxyFactoryBuilder) {
			customize(httpServiceProxyFactoryBuilder, Selectable.blank());
		}
