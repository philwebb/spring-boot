...
class AutoConfiguredHttpServiceProxyFactoryBuilders
	...
	public AutoConfiguredHttpServiceProxyFactoryBuilders(
			Stream<SelectableSet.Entry<HttpExchangeAdapter>> adapterEntries,
			List<HttpServiceProxyFactoryCustomizer> customizers) {
		super(adapterEntries::iterator, SelectableSet.Entry::selectable,
				(entry) -> asHttpServiceProxyFactoryBuilder(entry.element(), customizers));
	}

	private static HttpServiceProxyFactory.Builder asHttpServiceProxyFactoryBuilder(
			HttpExchangeAdapter httpExchangeAdapter, List<HttpServiceProxyFactoryCustomizer> customizers) {
