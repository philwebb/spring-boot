 ...
 *
 */
class OnHttpServiceProxyBeanCondition extends SpringBootCondition implements ConfigurationCondition {
	...
	public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
		ConditionMessage.Builder message = ConditionMessage.forCondition(ConditionalOnHttpServiceProxyBean.class);
		BeanFactory beanFactory = context.getBeanFactory();
		while (beanFactory != null) {
			if (beanFactory instanceof ConfigurableListableBeanFactory configurableListableBeanFactory
					&& hasHttpServiceProxyBeanDefinition(configurableListableBeanFactory)) {
				return ConditionOutcome.match(message.foundExactly("HTTP Service proxy bean"));
			}
		...
		return ConditionOutcome.noMatch(message.didNotFind("").items("HTTP Service proxy beans"));
	}
