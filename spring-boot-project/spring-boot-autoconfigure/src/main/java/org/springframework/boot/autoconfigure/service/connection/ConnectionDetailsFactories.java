...
public class ConnectionDetailsFactories {
	...
	// TODO Source is always a ServiceConnectedContainer so this will hold all factories
	// TODO Should we order the delegates to allow a custom factory to override?
	private static class CompositeConnectionDetailsFactory<S>
			implements ConnectionDetailsFactory<S, ConnectionDetails> {

		private final List<ConnectionDetailsFactory<S, ConnectionDetails>> delegates;

		public CompositeConnectionDetailsFactory(List<ConnectionDetailsFactory<S, ConnectionDetails>> delegates) {
