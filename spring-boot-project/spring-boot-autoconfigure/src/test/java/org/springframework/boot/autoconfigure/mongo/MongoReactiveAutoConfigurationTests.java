	...
	void configuresSslWithBundle() {
		this.contextRunner
			.withPropertyValues("spring.data.mongodb.ssl.bundle=test-bundle",
					"spring.ssl.keystore.test-bundle.key-store=classpath:test.jks",
					"spring.ssl.keystore.test-bundle.key-store-password=secret",
					"spring.ssl.keystore.test-bundle.key-password=password")
			.run((context) -> {
				SslSettings sslSettings = getSettings(context).getSslSettings();
				assertThat(sslSettings.isEnabled()).isTrue();
