...
class PemSslBundlePropertiesTests {
	...
	class SelectCertificateTests {
		...
		@Test
		void usingBeanSelectorSelectsNone() {
			testSelect(SelectCertificate.USING_BEAN_SELECTOR, -1);
		}
		...
		private void testSelect(SelectCertificate select, int expected) {
			...
			assertThat(selected).isEqualTo((expected != -1) ? candidates.get(expected) : null);
		}
	...
	class SelectPrivateKeyTests {
		...
		@Test
		void usingBeanSelectorSelectsNone() {
			PrivateKeyFileSelector selector = SelectPrivateKey.USING_BEAN_SELECTOR.getSelector();
			List<PrivateKeyFile> candidates = MockPrivateKeyFiles.create((files) -> files.add("a.key"));
			CertificateFile certificateFile = MockCertificateFiles.createSingle("b.crt");
