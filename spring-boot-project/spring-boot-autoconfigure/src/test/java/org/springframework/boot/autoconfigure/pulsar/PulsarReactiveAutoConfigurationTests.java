...
class PulsarReactiveAutoConfigurationTests {
	...
	// FilteredClassLoader(EnableReactivePulsar.class))
	// .run((context) ->
	// assertThat(context).doesNotHaveBean(XPulsarReactiveAnnotationDrivenConfiguration.class));
	// }
	...
	void beansAreInjectedInReactivePulsarSenderFactory() throws Exception {
		...
		try (ReactiveMessageSenderCache cache = mock(ReactiveMessageSenderCache.class)) {
			this.contextRunner.withPropertyValues("spring.pulsar.sender.topic-name=test-topic")
	...
	void reactiveListenerPropertiesAreHonored() {
		this.contextRunner.withPropertyValues("spring.pulsar.listener.schema-type=avro",
				"spring.pulsar.listener.handling-timeout=10s", "spring.pulsar.listener.use-key-ordered-processing=true",
	...
	class SenderCacheAutoConfigurationTests {
		...
		void cachingEnabledExplicitly() {
			this.contextRunner.withPropertyValues("spring.pulsar.sender.cache.enabled=true")
				.run(this::assertCaffeineProducerCacheProvider);
		}

		@Test
		void cachingCanBeDisabled() {
			this.contextRunner.withPropertyValues("spring.pulsar.sender.cache.enabled=false")
		...
		void cachingCanBeConfigured() {
			this.contextRunner.withPropertyValues("spring.pulsar.sender.cache.expire-after-access=100s",
					"spring.pulsar.sender.cache.expire-after-write=200s",
					"spring.pulsar.sender.cache.maximum-size=5150", "spring.pulsar.sender.cache.initial-capacity=200")
