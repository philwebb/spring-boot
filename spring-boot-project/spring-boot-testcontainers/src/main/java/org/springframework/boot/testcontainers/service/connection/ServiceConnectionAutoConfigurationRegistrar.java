...
class ServiceConnectionAutoConfigurationRegistrar implements ImportBeanDefinitionRegistrar {
	...
	private Set<ServiceConnection> getAnnotations(ListableBeanFactory beanFactory, String beanName,
	...
	private <C extends Container<?>> ContainerConnectionSource<C> createSource(
		...
		return new ContainerConnectionSource<>(beanName, origin, containerType,
				getContainerNameSupplier(beanDefinition), annotation,
				() -> beanFactory.getBean(beanName, containerType));
	}

	private Supplier<String> getContainerNameSupplier(BeanDefinition beanDefinition) {
		if (beanDefinition instanceof TestcontainerBeanDefinition testcontainerBeanDefinition) {
			return testcontainerBeanDefinition::getDockerImageName;
		}
		return () -> null;
	}
