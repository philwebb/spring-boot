 ...
 * @author Chris Bono
 */
final class SecurePulsarContainer extends PulsarContainer {

	static final int BROKER_TLS_PORT = 6651;

	static final int BROKER_HTTP_TLS_PORT = 8081;

	static SecurePulsarContainer withJksBasedTls() {
		return new SecurePulsarContainer(false);
	}

	static SecurePulsarContainer withPemBasedTls() {
		return new SecurePulsarContainer(true);
	}

	private SecurePulsarContainer(boolean pemBasedTls) {
		super(DockerImageNames.pulsar());
		...
		if (pemBasedTls) {
			...
			withEnv("PULSAR_PREFIX_brokerClientTlsKeyStoreType", "JKS");
			...
			withEnv("PULSAR_PREFIX_brokerClientTlsTrustStoreType", "JKS");
