...
class JdkFilteredProxySelector extends ProxySelector {
	...
	private final ProxySelector delecate;

	private final HttpClientInetAddressMatcher matcher;

	JdkFilteredProxySelector(ProxySelector delegate, HttpClientInetAddressMatcher matcher) {
		this.delecate = delegate;
	...
	public List<Proxy> select(URI uri) {
		...
		MatchingAddresses.of(Stream.of(host)).match(this::matchesResolvedHost).orElseThrow(host, this.matcher);
		return this.delecate.select(uri);
	}

	private boolean matchesResolvedHost(String host) {
		return this.matcher.matches(resolve(host));
	}
	...
	public void connectFailed(URI uri, SocketAddress address, IOException ex) {
		this.delecate.connectFailed(uri, address, ex);
	}
