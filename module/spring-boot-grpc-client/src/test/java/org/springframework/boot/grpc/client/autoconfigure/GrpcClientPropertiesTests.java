...
class GrpcClientPropertiesTests {
	...
	void defaultValues() throws Exception {
		...
		Channel channel = properties.getChannel().get("a");
		assertThat(channel.getUserAgent()).isNull();
		assertThat(channel.isBypassCertificateValidation()).isFalse();
		assertThat(channel.getInbound().getMessage().getMaxSize()).isEqualTo(DataSize.ofBytes(4194304));
		assertThat(channel.getInbound().getMetadata().getMaxSize()).isEqualTo(DataSize.ofBytes(8192));
		assertThat(channel.getDefault().getDeadline()).isNull();
		assertThat(channel.getIdle().getTimeout()).isEqualTo(Duration.ofSeconds(20));
		assertThat(channel.getKeepalive().getTime()).isEqualTo(Duration.ofMinutes(5));
		assertThat(channel.getKeepalive().getTimeout()).isEqualTo(Duration.ofSeconds(20));
		assertThat(channel.getKeepalive().isWithoutCalls()).isFalse();
		assertThat(channel.getSsl().getEnabled()).isNull();
		assertThat(channel.getSsl().getBundle()).isNull();
		assertThat(properties.getChannel().get("b").getTarget()).isEqualTo("static://localhost:9090");
	}
