...
[[io.rest-client.httpservice.autoconfigured]]
=== Auto-configured Interface Clients

By default, Spring Boot will scan for HTTP Service interfaces in all xref:using/auto-configuration.adoc#using.auto-configuration.packages[auto-configuration packages] and create beans for all interfaces that are annotated with javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation].

Our `EchoService` example above would be auto-configured as a client since it is annotated with javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation].


[TIP]
====
HTTP Service interfaces can be used to define a contract that can be used on both the client and the server.
To use these dual-purpose interfaces as clients, you will need to replace auto-configuration scanning with explicit javadoc:org.springframework.web.service.registry.ImportHttpServices[format=annotation] or javadoc:org.springframework.web.service.registry.AbstractHttpServiceRegistrar[] configuration.

See xref:io/rest-client.adoc#io.rest-client.httpservice.advanced-configuration[the "`Advanced Configuration`" section below] for more details.
====
...
You can also associate client customization logic to a group, such as adding code to insert required auth headers.

To directly associate an HTTP Service interface with a group, you can use the `group` or `value` attribute of javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation].
You can also specify a group when using javadoc:org.springframework.web.service.registry.ImportHttpServices[format=annotation] or javadoc:org.springframework.web.service.registry.AbstractHttpServiceRegistrar[].

To update our `EchoService` to use a group, we can write:

include-code::EchoService[]

We can then use a `base-url` property to link the `echo` group to an actual URL.
...
Spring Boot's javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation] auto-configuration may not be suitable for all applications.
For example, you may want to associate the same Java interface with multiple HTTP Service groups.
Or you might published shared HTTP Service interfaces for use across multiple applications, making it impossible to know in advance which group they should be associated with.

For these situations you can:

* Use the javadoc:org.springframework.web.service.registry.ImportHttpServices[format=annotation] annotation.
* javadoc:org.springframework.context.annotation.Import[format=annotation] your own javadoc:org.springframework.web.service.registry.AbstractHttpServiceRegistrar[] implementation.
...
As soon as you start to define your own HTTP Service client configuration, Spring Boot will no longer auto-configure scanning for javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation] annotated interfaces.
If you want to scan for javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation] annotated interfaces in addition to other HTTP Service client configuration, you can use the javadoc:org.springframework.boot.http.client.service.HttpServiceClientScan[format=annotation] annotation.

[NOTE]
====
You should not use javadoc:org.springframework.boot.http.client.service.HttpServiceClient[format=annotation] on HTTP Service interfaces that you directly import or register.
Doing so may cause an javadoc:org.springframework.boot.http.client.service.HttpServiceClientGroupMismatchException[] exception to be thrown.

This is due to the fact that Spring has no clear signal as to which group should be used when there is a clash.

For example, given the following HTTP Service client interface:

include-code::MyHttpService[]

The following configuration is ambiguous:

include-code::MyHttpServiceConfiguration[]

Here, the `@HttpServiceClient` annotation is indicating that that the `greeting` group should be used.
Where as the `@ImportHttpService` is indicating that the `default` group should be used.

The `@ImportHttpService` group is particularly easy to miss, since `default` is used when no explicit `group` attribute is declared.
====
