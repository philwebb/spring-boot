...
Client authentication can also be enforced by setting configprop:spring.grpc.server.ssl.client-auth[] to `optional` or `require`.
...
Clients can call remote gRPC services by importing one or more of the "`sub`" classes generated from their `.proto` file.
