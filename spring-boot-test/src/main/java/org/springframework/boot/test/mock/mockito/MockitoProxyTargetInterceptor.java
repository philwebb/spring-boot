 ...
 * @author Phillip Webb
 */
class MockitoProxyTargetInterceptor implements MethodInterceptor {
	...
	MockitoProxyTargetInterceptor(Object source, Object target) throws Exception {
		this.source = source;
		this.target = target;
	...
	public static void applyTo(Object source) {
		...
		try {
			...
			for (Advisor advisor : advised.getAdvisors()) {
				if (advisor instanceof MockitoProxyTargetInterceptor) {
					return;
				}
			}
			Object target = AopTestUtils.getUltimateTargetObject(source);
			Advice advice = new MockitoProxyTargetInterceptor(source, target);
