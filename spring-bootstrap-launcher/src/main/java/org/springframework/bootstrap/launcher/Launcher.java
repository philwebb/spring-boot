...
public abstract class Launcher {
	...
	/**
	 * The main runner class. This must be loaded by the created ClassLoader so cannot
	...
	/**
	 * Launch the application. This method is the initial entry point that should
	 * be called by a subclass {@code public static void main(String[] args)} method.
	...
	protected void launch(String[] args, ProtectionDomain protectionDomain)
		...
		String codeSourcePath = (codeSourceLocation == null ? null
				: codeSourceLocation.getPath());
	...
	/**
	 * Determine if the specified {@link JarEntry} is a nested item that should
	 * be added to the classpath. The method is called once for each entry.
	...
	/**
	 * Called to post-process lib entries before they are used. Implementations
	 * can add and remove entries.
