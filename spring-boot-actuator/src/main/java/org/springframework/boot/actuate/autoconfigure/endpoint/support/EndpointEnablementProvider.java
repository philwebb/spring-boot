...
public class EndpointEnablementProvider {
	...
	public EndpointEnablement getEndpointEnablement(String endpointId,
		...
		Assert.isTrue(!endpointId.equals("all"), "Endpoint id 'all' is a reserved value "
	...
	private EndpointEnablement getGlobalEndpointEnablement(String endpointId,
		...
		result = findEnablement(getKey("all", "enabled"));
		if (result != null) {
			return result;
		}
	...
	private EndpointEnablement findGlobalEndpointEnablement(
			EndpointDelivery delivery) {
		if (delivery != null) {
			EndpointEnablement result = findEnablement(getKey("all", delivery));
			if (result != null) {
				return result;
			}
			if (!delivery.isEnabledByDefault()) {
				return getDefaultEndpointEnablement("all", false, delivery);
			}
			return null;
		}
		return findEnablementForAnyDeliveryTechnology("all");
	}
