 ...
 * @author pwebb
 */
public class ThingToPushClassUpdates {

	// Will listen for events then push CLFs up to the remote thinggy

	private ClassPathChangedEvent event;

	private ClassLoaderFile file;

	public void dunno(ClassPathChangedEvent event) {
		for (ChangedFiles changedFiles : event.getChangeSet()) {
			changedFiles.getSourceFolder();
			for (ChangedFile changedFile : changedFiles) {
				changedFile.getRelativeName();
				changedFile.getFile(); // to bytes
				changedFile.getType(); // adapt
			}
		}
