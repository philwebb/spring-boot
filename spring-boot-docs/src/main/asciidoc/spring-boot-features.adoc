...
* Spring Test & Spring Boot Test -- utilties and integration test support for Spring Boot
  applications.
...
* JSONassert -- An assertion library for test JSON
...
`spring-test` `@ContextConfiguration` annotation. All three work by creating the
`ApplicationContext` used in your tests via `SpringApplication` so that you get all the
additional Spring Boot features.

The specific annotation that choose will depend on the type of test that you are writings:

* `@SpringApplicationTest` -- Loads the application context using `SpringApplication` and
  provides a mock servlet environment when appropriate. Embedded servlet containers are
  not started when using this annotation.
* `@WebIntegrationTest` -- Loads the context using `SpringApplication` and provides a real
  servlet environment. Embedded servlet containers are started and listening on a defined
  or random port.
* `@IntegrationTest` -- Loads the context using `SpringApplication` but does not provides
  any servlet environment. Useful for integration testing non-web applications.
...
When testing Spring Boot applications this is often no longer required.
...
potentially time consuming process of loading the context will only happen once, regardless
of the number of tests that actually run.
...
Use the `@SpringApplicationTest` annotation to load an `ApplicationContext` via
`SpringApplication` and configure it with a mock servlet environment when appropriate.
...
Use the `@WebIntegrationTest` annotation to load an `WebApplicationContext` via
`SpringApplication` and configure it with fully running server listening on its normal
or a random port.
...
for a bean. You can use the annotation to add new beans, or replace a single existing bean
definition. The annotation can be used directly on test classes, on fields within your
test; or on `@Configuration` classes and fields. When used on a field the, instance of the
created mock will also be injected. Mock beans are automatically reset after each test
method.
...
a little too much for tests. It's often helpful to load only parts of the configuration
that are required to test a '`slice`' of your application. For example, you might want
to test that Spring MVC controllers are mapping URLs correctly, and you don't want to
involve and database calls in those tests; or you _might be wanting_ to test JPA entites,
and you're not interested in web layer when those tests run.
...
providing a `@...Test` annotation to be used on the test class and one or more
`@AutoConfigure...` annotations that can be used to trigger and customize
auto-configuration.
...
and `@JsonComponent` beans and any just Jackson `Modules`. It also configures `Gson`
...
helper fields on the test class will be automatically configured when using `@JsonTest`.
