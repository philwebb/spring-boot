...
public class DispatcherServletAutoConfiguration {
	...
	private static class DefaultDispatcherServletCondition extends SpringBootCondition {
		...
		public ConditionOutcome getMatchOutcome(ConditionContext context,
			...
			ConditionMessage.WithCondition message = ConditionMessage
			...
			if (dispatchServletBeans.contains(DEFAULT_DISPATCHER_SERVLET_BEAN_NAME)) {
				return ConditionOutcome.noMatch(message.found("dispatcher servlet bean")
						.in(DEFAULT_DISPATCHER_SERVLET_BEAN_NAME));
			}
	...
	private static class DispatcherServletRegistrationCondition
		...
		private ConditionOutcome checkServletRegistration(
				ConfigurableListableBeanFactory beanFactory) {
			ConditionMessage.WithCondition message = startMessage();
			...
			if (registrations.isEmpty()) {
				if (containsDispatcherRegistrationBean) {
					return ConditionOutcome
							.noMatch(message.found("non servlet registration bean").in(
			...
			if (registrations
				...
				return ConditionOutcome.noMatch(message.found("servlet registration bean")
						.in(DEFAULT_DISPATCHER_SERVLET_REGISTRATION_BEAN_NAME));
			}
			if (containsDispatcherRegistrationBean) {
				return ConditionOutcome
						.noMatch(message.found("non servlet registration bean")
								.in(DEFAULT_DISPATCHER_SERVLET_REGISTRATION_BEAN_NAME));
			}
			return ConditionOutcome.match(message.found("servlet registration beans")
					.in(registrations).append("and none is named "
							+ DEFAULT_DISPATCHER_SERVLET_REGISTRATION_BEAN_NAME));
		}

		private ConditionMessage.WithCondition startMessage() {
