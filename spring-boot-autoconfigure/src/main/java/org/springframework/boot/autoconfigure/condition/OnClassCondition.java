		...
		if (onClasses != null) {
			...
			if (!missing.isEmpty()) {
				return ConditionOutcome.noMatch(ConditionMessage
						...
						.didNotFind("required class", "required classes").in(missing));
			}
			matchMessage = matchMessage.andCondition(ConditionalOnClass.class)
					.found("required class", "required classes")
					.in(getMatchingClasses(onClasses, MatchType.PRESENT, context));
		}
			...
			if (!present.isEmpty()) {
				return ConditionOutcome.noMatch(
						ConditionMessage.forCondition(ConditionalOnMissingClass.class)
								.found("unwanted class", "unwanted classes").in(present));
			}
			matchMessage = matchMessage.andCondition(ConditionalOnMissingClass.class)
					.didNotFind("unwanted class", "unwanted classes")
					.in(getMatchingClasses(onMissingClasses, MatchType.MISSING, context));
		}
