...
import org.springframework.boot.autoconfigure.condition.ConditionMessage.WithCondition;
...
public abstract class ResourceCondition extends SpringBootCondition {
	...
	public ConditionOutcome getMatchOutcome(ConditionContext context,
		...
		if (resolver.containsProperty(this.propertyName)) {
			return ConditionOutcome.match(startConditionMessage()
					.foundExactly("proprty " + this.prefix + this.propertyName));
		}
	...
	protected ConditionOutcome getResourceOutcome(ConditionContext context,
		...
		if (found.isEmpty()) {
			ConditionMessage message = startConditionMessage()
					...
					.in(Arrays.asList(this.resourceLocations));
	...
	protected final WithCondition startConditionMessage() {
