...
public class CassandraProperties {
	...
	public Cluster createCluster() {
		Cluster.Builder builder = Cluster.builder()
				.withClusterName(this.getClusterName()).withPort(this.getPort());
		...
		if (!StringUtils.isEmpty(this.getLoadBalancingPolicy())) {
			try {
				Class loadBalancingPolicyClass = ClassUtils.forName(
						this.getLoadBalancingPolicy(), null);
		...
		if (this.getConsistency() != null) {
			ConsistencyLevel consistencyLevel = ConsistencyLevel.valueOf(this
					.getConsistency());
			queryOptions.setConsistencyLevel(consistencyLevel);
		}
		if (this.getSerialConsistency() != null) {
			ConsistencyLevel serialConsistencyLevel = ConsistencyLevel.valueOf(this
		...
		if (!StringUtils.isEmpty(this.getReconnectionPolicy())) {
			try {
				Class reconnectionPolicyClass = ClassUtils.forName(
						this.getReconnectionPolicy(), null);
		...
		builder.addContactPoints(StringUtils.commaDelimitedListToStringArray(this
				.getContactPoints()));
