...
class SpringFactoriesEnvironmentPostProcessorsFactory implements EnvironmentPostProcessorsFactory {
	...
	public List<EnvironmentPostProcessor> getEnvironmentPostProcessors(DeferredLogFactory logFactory,
		...
		postProcessors.addAll(this.loader.load(org.springframework.boot.env.EnvironmentPostProcessor.class));
		AnnotationAwareOrderComparator.sort(postProcessors);
		return postProcessors.stream().map(Adapter::apply).toList();
	}

	static record Adapter(
