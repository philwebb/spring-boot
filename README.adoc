...
=== Proxy
When you need a HTTP proxy to access the Internet, you need to specify the Java proxy
parameters together with the Maven `proxy` profile:
...
Maven Surefire plugin. Several Spring Boot tests download external packages and without
passing the proxy settings to the forked VM, those test-cases are going to fail.
...
You need to exclude the clojars.org repository by adding a mirror to your user's
