...
* For the "`Project`" click on `+` to add a new setup file. Select "`Github Projects`"
  and browser for `/eclipse/spring-boot-project.setup` from your locally cloned copy of
  the source code. Click OK to add the setup file to the list.
* Double-click on "`Spring Boot`" from the project list to add it to the list that will
  be provisioned then click "`next`".
* Click show all variables and make sure that "`Checkout Location`" points to your locally
  cloned copy of the source code. You might also want to pick a different install location
  here.
