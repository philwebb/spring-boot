...
/**
 * Tests for {@link ByteBufferRandomAccessData}.
 * 
 * @author Phillip Webb
 */
public class ByteBufferRandomAccessDataTests {
	...
	private ByteBufferRandomAccessData file;
	...
	public void setup() throws Exception {
		...
		outputStream.close();
		MappedByteBuffer buffer = new RandomAccessFile(this.tempFile, "r").getChannel()
				.map(MapMode.READ_ONLY, 0, this.tempFile.length());
		this.file = new ByteBufferRandomAccessData(buffer);
		...
		for (int i = 0; i < 100; i++) {
			results.add(executorService.submit(new Callable<Boolean>() {
				...
				public Boolean call() throws Exception {
					InputStream subsectionInputStream = ByteBufferRandomAccessDataTests.this.file
							.getSubsection(0, 256)
