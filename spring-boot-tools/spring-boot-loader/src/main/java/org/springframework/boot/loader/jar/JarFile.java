...
public class JarFile extends java.util.jar.JarFile implements Iterable<JarEntryData> {
	...
	public JarEntryData getJarEntryData(String name) {
		...
		AsciiBytes key = new AsciiBytes(name);
		JarEntryData entryData = entriesByName.get(key);
		if (entryData == null && !name.endsWith("/")) {
			entryData = entriesByName.get(key.append(SLASH));
		}
		return entryData;
	}
	...
	public synchronized JarFile getNestedJarFile(final JarEntryData sourceEntry)
		...
		try {
			if (sourceEntry.nestedJar == null) {
				sourceEntry.nestedJar = (sourceEntry.isDirectory() ? getNestedJarFileFromDirectoryEntry(sourceEntry)
						: getNestedJarFileFromFileEntry(sourceEntry));
			}
